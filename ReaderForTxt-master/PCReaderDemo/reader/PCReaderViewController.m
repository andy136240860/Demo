...
- (void)setupToolbar
{
    ...
    [self.toolbar setItems:@[item1,fixibleItem,item2,fixibleItem,item3,fixibleItem,fixibleItem,item5]];
...
{
    [PCGlobalModel shareModel].isNightModel = ![PCGlobalModel shareModel].isNightModel;
...
- (void)changeNightModelAction:(BOOL)isNightModel
{
    [PCGlobalModel shareModel].isNightModel = isNightModel;
    //更新字体
    if ([self.delegate respondsToSelector:@selector(adjustRangeArrayForText)]) {
        [self.delegate adjustRangeArrayForText];
    }
